<#-- 
================================================================================
 FACTOR GENERATOR: 1:M RELATIONSHIP MATRIX (MULTI-DATABASE)
================================================================================
 Template: factor.combination.pk.multidatabase.columns.fk.1toM.ftl
 Version:  1.3.0
 Purpose:  Generates parent-child table pairs with 1:M relationships.
           Uses 'R1MP_' and 'R1MC_' prefixes for schema coexistence.
 
 Configuration (CLI):
   ${PRV_SYS_GEN_TYPE|one|Private Gen list: one, many, copy}
   ${PRV_SYS_GEN_FILENAME|factor.combination.pk.multidatabase.columns.fk.1toM.sql|Output filename}
...
<#-- 
  Macro: generateTable
  Purpose: Generates parent-child table pairs with 1:M foreign key relationships.
-->
    ...
    <#if !dbType?has_content || currentDb == dbType>
      ...
      <#list 1..dataTypes?size as combinationLength>
        ...
        <#list combinations as combination>
          ...
          <#-- Using Relationship 1:M Parent / Child prefixes -->
          <#local parentName = shortenIdentifierEnterprise("R1M_" + typeSuffix + "_P", 30)>
          <#local childName  = shortenIdentifierEnterprise("R1M_" + typeSuffix + "_C", 30)>

-- ================================================================================
-- RELATIONSHIP [1:M]: ${parentName} (Parent) <-> ${childName} (Child)
...
${getCreateTableStatement(childName, currentDb)} (
          <#local identityCountC = 0>
          <#list combination as type>
            <#local columnDefC = type>
            <#if isIdentityColumn(type, currentDb)>
              <#local identityUsedC = (identityCountC > 0)>
              <#-- SQLite specific: cannot have column-level identity/PK if table-level PK is used -->
              <#if currentDb == "sqlite" || identityUsedC>
                <#local columnDefC = replaceIdentityColumn(type, currentDb)>
              <#else>
                <#local identityCountC = identityCountC + 1>
              </#if>
            </#if>
    ${getColumnName(type, type?index + 1)} ${columnDefC} NOT NULL,
          </#list>
    -- Foreign Key Reference Columns (1:M)
<#list combination as type>
    parent_${getColumnName(type, type?index + 1)} ${replaceIdentityColumn(type, currentDb)} NOT NULL,
</#list>
<@generateColumn dbType=currentDb alreadyHasIdentity=(identityCountC > 0)/><#rt>
<#t>
    PRIMARY KEY (
<#list combination as type>
        ${getColumnName(type, type?index + 1)}<#sep>, 
