<#-- 
================================================================================
 FACTOR GENERATOR: PK & COLUMNS MATRIX (MULTI-DATABASE)
================================================================================
 Template: factor.combination.pk.multidatabase.columns.ftl
 Version:  1.3.0
 Purpose:  Generates SQL schema matrices including primary keys and audit columns.
           Uses 'MCL_' prefix to allow coexistence in a single schema.
 
 Configuration (CLI):
   ${PRV_SYS_GEN_TYPE|one|Private Gen list: one, many, copy}
   ${PRV_SYS_GEN_FILENAME|factor.combination.pk.multidatabase.columns.sql|Output filename}
...
<#-- 
  Macro: generateTable
  Purpose: Orchestrates the generation of tables with composite PKs and standard columns.
-->
    ...
    <#if !dbType?has_content || currentDb == dbType>
      ...
      <#list 1..dataTypes?size as combinationLength>
        ...
        <#list combinations as combination>
          ...
          <#-- Construct descriptive base name with Matrix Column prefix -->
          <#local baseName = "MCL_${combinationLength}_${abbrevs?join('_')}">
          <#local tableName = shortenIdentifierEnterprise(baseName, 30)>

-- ================================================================================
-- MATRIX [PK + COLUMNS]: ${tableName}
-- SCHEMA: Composite PK (${combination?join(", ")}) + Standard Columns
-- ================================================================================
